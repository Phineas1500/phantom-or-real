...
| `positive_control_steering` | runnable pending gpu run | yes | no | Run `sbatch scripts/stage2_positive_control_verbosity_27b_L45.sbatch` before interpreting new steering nulls. |
...
| 2 | Run the true positive-control steering gate; the historical answer-property smoke did not pass. | `sbatch scripts/stage2_positive_control_verbosity_27b_L45.sbatch` |
...
- The runnable positive-control gate is `scripts/stage2_positive_control_verbosity_27b_L45.sbatch` and writes `docs/positive_control_verbosity_gemma3_27b_l45.json`.
