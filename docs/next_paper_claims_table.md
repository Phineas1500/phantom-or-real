...
| 12 | Cross-task: the behavioral story replicates (recognition gap 33/48; candidates 1.000; hint-first 0.875) but context-encoding patch accessibility does NOT (full patch +0.039 null vs property +0.491; clean random control). Pre-registered bin: claims 5-6 scope to property; pathway-mix account pre-registered — subtype hint rides the token route. | landed (task-dependence bin) | docs/subtype_localization_patch_27b_summary.md; job 457005 | subtype gold-KV arm queued as the double-dissociation test; discussion gains task-dependence subsection |
