...
| 7 | Necessity of concept-position encodings is NULL: the hinted repair survives their reversion (1.000, exact zero delta vs in-job hinted baseline; random control identical). Conditional link 1 fired: the literal hint tokens are a parallel decode-time pathway. | landed (null, pre-registered routing) | job 457002 arms 2-4; docs/focus_state_composite_27b_property_summary.md | KV/hint-span job jumps the queue (4 arms incl. hint-span ablation + gold-KV machinery control) |
...
| 9 | Geometry: the causal focus content lies outside the readable correctness subspace — survives restriction. | landed | concept-position deltas project at 0.013-0.014 vs chance 0.041, all three layers | none; safe as paper prose |
