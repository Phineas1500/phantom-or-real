...
| Dictionary/dark-matter tests | Gemma L45 local MLP-out S1/S3 and local MLP-in-weighted -> MLP-out transcoder S1 are complete; next compare against residual-site local dictionaries or public Gemma Scope MLP-out/transcoder artifacts to separate site/basis effects from local-training effects. | Determine whether predictive signal moves from error into reconstruction in the right basis/objective. |
