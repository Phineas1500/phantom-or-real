...
| Dictionary/dark-matter tests | Gemma L45 local MLP-out S1/S3, local residual S1, and local MLP-in-weighted -> MLP-out transcoder S1 are complete; next compare against public Gemma Scope MLP-out/transcoder artifacts or repeat local residual on S3 to separate public-artifact effects from split robustness. | Determine whether predictive signal moves from error into reconstruction in the right basis/objective. |
