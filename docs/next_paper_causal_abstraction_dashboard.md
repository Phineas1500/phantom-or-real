...
| Weak correctness is readable but much less height-stable than strong correctness. | `free_form_correctness` | `raw_direction` | first_pass_completed | fixed-C target/OOD pass: weak-correctness S1/S3 AUCs Gemma property=0.808/0.802, Gemma subtype=0.757/0.711, Qwen property=0.794/0.747, Qwen subtype=0.822/0.836; weak h1/h2 -> h3/h4 AUCs only 0.606-0.655, while strong height AUCs are 0.725/0.725 for Gemma and 0.838/0.786 for Qwen |
...
| Target/OOD preflight and weak-target probes | first_pass_completed | docs/target_ood_preflight.md; docs/target_ood_probe_findings.md; docs/target_ood_raw_probe_27b_weak_main.json; docs/target_ood_raw_probe_27b_strong_height.json | Weak correctness is readable on S1/S3 but generalizes poorly from h1/h2 to h3/h4; strong correctness height extrapolation is materially better, especially for Qwen. Binary perfect-quality mostly collapses to strong correctness. |
...
| Target and OOD extensions | First weak-correctness and height-extrapolation pass is complete; next add a graded quality-score regression/ordinal probe only if we want a true parsimony target, or generate Qwen name-scramble artifacts if cross-model OOD controls become central. | Separate correctness, parsimony, depth/difficulty, and name-familiarity components. |
...
- Treat fixed-`C=1.0` target/OOD probe results as planning evidence; rerun full C-grid probes with bootstrap CIs before manuscript-level numerical claims.
