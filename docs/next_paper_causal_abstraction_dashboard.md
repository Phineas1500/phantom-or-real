...
| Gemma local residual dictionary closes the public-SAE dark-matter gap, so local objective/capacity matters beyond MLP-output site choice. | `free_form_correctness` | `reconstruction` | completed | job=456721; local top-k residual dictionary S1 energy property=0.998, subtype=0.998; target/reconstruction/error AUCs: property=0.908/0.890/0.768, subtype=0.930/0.916/0.828. Public Gemma Scope residual SAEs at L45 leave stronger error probes than reconstruction probes, while local residual, local MLP-out, and local transcoder dictionaries all move most signal into reconstruction. |
...
| Dictionary/dark-matter local residual contrast | completed | docs/gemma3_local_resid_reconstruction_probe_27b_l45_w4096_k64_s1.json | Local residual dictionary also moves most predictive signal into reconstruction, unlike public residual Gemma Scope SAEs; this points to artifact objective/capacity/training recipe as a major part of the dark-matter gap. |
...
| Dictionary/dark-matter tests | Gemma L45 local MLP-out S1/S3, local residual S1, local MLP-in-weighted -> MLP-out transcoder S1, and public residual/MLP-out/transcoder comparisons are complete; next repeat local residual on S3 or test a denser/local-public hybrid before treating the local objective/capacity diagnosis as split-robust. | Determine whether predictive signal moves from error into reconstruction in the right basis/objective. |
