#!/usr/bin/env python3
"""Item L-triple-prime pooled verdict (fresh-draw replication): the composition (gauge-selected branch x
...
Writes docs/selfaddress_l3_27b_property_pooled.json.
"""
...
OUT = Path("docs/selfaddress_l3_27b_property_pooled.json")
...
def main() -> int:
    ...
    # Fresh rows: no L1-recorded bestofN or verbatim reference exists (stated in
    # the L3 registration). Comparator = majority/any over the 8 in-job baseline
    # samples per row (self-consistency@8), labeled as an adaptation.
    bestofn = {r: v for r, v in baseline.items()}
    shared, verbatim = [], 0
    ...
    report: dict = {
        ...
        "baseline_verbatim_gate": {
            ...
            "pass": None, "note": "no shared rows with prior jobs by design (fresh draw)",
        },
    }
