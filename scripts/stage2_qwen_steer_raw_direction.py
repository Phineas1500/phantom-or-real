#!/usr/bin/env python3
"""Run a Qwen HF raw residual direction steering pilot.

This mirrors ``stage2_steer_raw_direction.py`` without TransformerLens. The
hook site is ``model.model.layers[L]``, matching the Qwen-Scope residual
extraction path used elsewhere in the Qwen replication branch.
"""
    ...
    *,
    ...
    def hook_fn(_module, _inputs, output):
        ...
        if direction_tensor is None:
            direction_tensor = torch.as_tensor(vector, device=hidden.device, dtype=hidden.dtype)
            cache[key] = direction_tensor
        patched = hidden.clone()
        patched[:, -1, :] = patched[:, -1, :] + float(delta) * direction_tensor
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--probe-seed", type=int, default=20260524)
    parser.add_argument("--orthogonal-seed", type=int, default=20260525)
    ...
    parser.add_argument("--out-jsonl", type=Path, default=Path("results/stage2/steering/qwen35_raw_l45_property_steering_pilot.jsonl"))
    parser.add_argument("--direction-output", type=Path, default=Path("results/stage2/steering/qwen35_raw_l45_property_steering_direction.npz"))
    parser.add_argument("--output", type=Path, default=Path("docs/qwen35_raw_steering_27b_l45_property_pilot.json"))
    return parser
...
def main() -> int:
    ...
    print("Qwen HF raw-direction steering pilot", flush=True)
    print(f"model={args.model} task={args.task} layer={args.layer}", flush=True)
    ...
    save_direction_artifact(path=args.direction_output, direction=direction, orthogonal_direction=orthogonal_direction)
    print(
        "direction: "
    ...
    report = {
        ...
        "script": "scripts/stage2_qwen_steer_raw_direction.py",
        "model": args.model,
