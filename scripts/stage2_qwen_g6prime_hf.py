#!/usr/bin/env python3
"""Item G6' (HF pathway): raw-transfer control battery (the G6 nail-down).

Identical rows/class-mean/norms to G6. PRIMARY = raw96 CI excludes zero AND
paired (raw96 - shuffled-raw family) CI excludes zero. Gates: unhinted,
rank16, AND class_mean_raw96 verbatim vs G6 (job 458474). New arms:
shuffled-raw x2, signflip-raw, rand-perpos d2/d3, shuffled-proj64 at
64-norms. Pre-registered: docs/causal_handle_directions.md item G6'."""
...
from scripts.stage2_rank_k_guard_v2 import Arm, select_fresh_rows, class_vector_from_labels, shuffled_label_vectors  # noqa: E402
...
def build_g6prime_arms(layer: int) -> list[tuple[Arm, int]]:
    """(arm, seed_ai) pairs; seed_ai 0/10/33 pin the G6 replication gates."""
    return [
        ...
        (Arm(f"shuffled_raw96_L{layer}_d1", "shuf_raw_add", "unhinted_baseline", (layer,), 16, "shuffled_label_classmean96_raw"), 40),
        (Arm(f"shuffled_raw96_L{layer}_d2", "shuf_raw_add", "unhinted_baseline", (layer,), 16, "shuffled_label_classmean96_raw"), 41),
        (Arm(f"signflip_raw96_L{layer}", "signflip_raw_add", "unhinted_baseline", (layer,), 16, "negated_classmean96_raw"), 42),
        (Arm(f"rand_norm_perpos_add_L{layer}_d2", "rand_dir_add", "unhinted_baseline", (layer,), 16, "random_direction_perpos_norm"), 43),
        (Arm(f"rand_norm_perpos_add_L{layer}_d3", "rand_dir_add", "unhinted_baseline", (layer,), 16, "random_direction_perpos_norm"), 44),
        (Arm(f"shuffled96_proj64_at64norms_L{layer}_d1", "shuf_proj64_add", "unhinted_baseline", (layer,), 64, "shuffled_label_classmean96_projected64"), 45),
    ]
...
def main() -> int:
    ...
    p.add_argument("--out-jsonl", type=Path, default=Path("results/stage2/erasure/qwen_g6prime_ladder.jsonl"))
    p.add_argument("--output", type=Path, default=Path("docs/qwen_g6prime_ladder.json"))
    args = p.parse_args()
    started = time.time()
    ...
    shuf_raw_vectors = shuffled_label_vectors(row_means, labels, draws=2, seed=args.control_seed + 10)
    ...
    fi_analog = {"skipped": "identical inputs computed in G6 (job 458474)"}
    ...
    def arm_vector(arm: Arm, prep: dict) -> np.ndarray:
        ...
        if arm.kind == "shuf_raw_add":
            draw = int(arm.label.rsplit("_d", 1)[1])
            return norm_matched(shuf_raw_vectors[draw - 1], prep, arm.rank_k)
        if arm.kind == "signflip_raw_add":
            return norm_matched(-class_vector, prep, arm.rank_k)
        if arm.kind == "shuf_proj64_add":
            components = basis_for(arm.rank_k, prep["ri"])["components"]
            direction = (shuf_raw_vectors[0] @ components.T) @ components
            return norm_matched(direction, prep, arm.rank_k)
                ...
                try:
                    for s in range(args.samples_per_row):
                        ...
                        o = {"schema_version": 1, "source_row_index": prep["ri"],
                             ...
                             "method": "qwen_g6prime_control_battery", "patch_layer": L,
