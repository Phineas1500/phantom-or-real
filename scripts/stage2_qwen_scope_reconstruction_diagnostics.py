#!/usr/bin/env python3
"""Decode Qwen-Scope top-k features and probe reconstruction/error activations."""
...
from src.stage2_reconstruction import ReconstructionStats, dense_topk_features  # noqa: E402
from src.stage2_sae import display_path, snapshot_revision_from_path  # noqa: E402
...
def verify_sparse_decode(
    *,
    ...
    with torch.inference_mode():
        sparse_recon = decode_qwen_scope_topk(sae, indices, values, dtype=dtype).float()
        ...
        dense_recon = (dense_features @ sae.W_dec.T + sae.b_dec).float()
...
def decode_one_dataset(
    *,
    ...
    if raw_activations.shape[1] != sae.d_model:
        raise ValueError(f"raw dim {raw_activations.shape[1]} != SAE d_model {sae.d_model}")
    ...
    for start in range(0, raw_activations.shape[0], chunk_size):
        ...
        with torch.inference_mode():
            reconstruction = decode_qwen_scope_topk(sae, indices_chunk, values_chunk, dtype=dtype)
    ...
    common_meta = {
        ...
        "sae_repo_id": feature_meta.get("sae_repo_id"),
...
def serializable_probe_result(result: dict[str, Any]) -> dict[str, Any]:
    return {key: value for key, value in result.items() if not key.startswith("_artifact_")}
...
def run_diagnostics(
    *,
    ...
    report: dict[str, Any] = {
        ...
        "sae_id": sae_id,
        "top_k": top_k,
        "hf_revision_requested": hf_revision,
    ...
    for task in tasks:
        ...
        for kind, prefix in (("reconstruction", recon_prefix), ("error", error_prefix)):
            probe_results[kind] = serializable_probe_result(
                run_raw_activation_probe(
...
def main() -> None:
    ...
    for task, result in report["results"].items():
        ...
        print(f"{task}: recon_auc={recon_auc} error_auc={error_auc} energy={energy}")
