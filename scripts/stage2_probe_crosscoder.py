#!/usr/bin/env python3
"""Train logistic probes on sparse crosscoder feature files."""
...
from src.stage2_crosscoder import crosscoder_feature_prefix  # noqa: E402
...
def load_crosscoder_dataset(
    *,
    ...
    x_all = topk_tensors_to_csr(
        tensors["top_indices"],
        tensors["top_values"],
        d_sae=int(meta["crosscoder_cfg"]["d_sae"]),
    )
...
def run_crosscoder_probe_grid(
    *,
    ...
    for task in tasks:
        ...
        dataset = load_crosscoder_dataset(feature_prefix=prefix, drop_parse_failed=drop_parse_failed)
        source_file = dataset["meta"]["source_file"]
        ...
        result.update(
            {
                ...
                "d_sae": int(dataset["meta"]["crosscoder_cfg"]["d_sae"]),
                "crosscoder_id": dataset["meta"]["crosscoder_id"],
...
def main() -> None:
    ...
    report = run_crosscoder_probe_grid(
